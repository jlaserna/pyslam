...
sys.path.append("/src/UPMResearchCAR-Robocity/imageMatcher/projects/pyslam")
...
#
if test_type == 'graf': 
    img1 = cv2.imread('/src/UPMResearchCAR-Robocity/imageMatcher/projects/pyslam/test/data/graf/img1.ppm') # queryImage
    img2 = cv2.imread('/src/UPMResearchCAR-Robocity/imageMatcher/projects/pyslam/test/data/graf/img3.ppm') # trainImage   img2, img3, img4
...
preBucketing = copy.deepcopy(img1)

kps, des = feature_tracker.detectAndCompute(preBucketing)

# Convert from list of keypoints to an array of points 
kpts = np.array([x.pt for x in kps], dtype=np.float32)

#kpsSub = copy.deepcopy(kps)

for pts in kpts:
    pt = np.rint(pts).astype(int)
    a,b = pt.ravel()
    cv2.circle(preBucketing,(a,b), 2, (0, 255, 0), -1)
...
xLen = int(postBucketing.shape[1] / N)
yLen = int(postBucketing.shape[0] / M)

finalKps = list()

for i in range(N):
    for j in range(M):
        SubImage = copy.deepcopy(postBucketing[(j * yLen):((j + 1) * yLen), (i * xLen):((i + 1) * xLen)])
...
for pts in finalKps:
    a = pts[0]
    b = pts[1]
    cv2.circle(postBucketing,(a,b), 2, (0, 255, 0), -1)

subImage = copy.deepcopy(img1)
...
for pts in finalKps:
    a = int(pts[0])
    b = int(pts[1])
    cv2.circle(subImage,(a,b), 2, (0, 255, 0), -1)


for i in range(N):
    cv2.line(postBucketing, (i * xLen, 0), (i * xLen, 640), (255, 255, 255), 1)
    cv2.line(subImage, (i * xLen, 0), (i * xLen, 640), (255, 255, 255), 1)
...
MPlotFigure(subImage, title='Post-Bucketing (Simple)')
MPlotFigure.show()
